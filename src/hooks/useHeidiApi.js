...
import {
  ...
  getSession,
  updateSession,
} from "../lib/heidi-api/endpoints/sessions.js";
import {
...
import {
  getConsultNoteTemplates,
  generateConsultNote,
} from "../lib/heidi-api/endpoints/notes.js";
...
export function useHeidiApi(apiKey) {
  ...
  const updateSessionDetails = useCallback(
    async (sessionId, sessionData) => {
      ...
      return handleApiCall(() =>
        updateSession(client, token, sessionId, sessionData)
      );
    },
    [client, token, handleApiCall]
  );
  ...
  const startTranscription = useCallback(
    async (sessionId) => {
      if (!token) throw new Error("Must authenticate first");
      return handleApiCall(() =>
        initializeTranscription(client, token, sessionId)
      );
    },
    [client, token, handleApiCall]
  );
  ...
  const endTranscription = useCallback(
    async (sessionId, recordingId) => {
      if (!token) throw new Error("Must authenticate first");
      return handleApiCall(() =>
        finishTranscription(client, token, sessionId, recordingId)
      );
    },
    [client, token, handleApiCall]
  );
  ...
  const generateNote = useCallback(
    async (sessionId, noteOptions) => {
      if (!token) throw new Error("Must authenticate first");
      return handleApiCall(() =>
        generateConsultNote(client, token, sessionId, noteOptions)
      );
    },
    [client, token, handleApiCall]
  );
  ...
  const askHeidiAI = useCallback(
    async (sessionId, aiCommandText, content, contentType = "MARKDOWN") => {
      if (!token) throw new Error("Must authenticate first");
      return handleApiCall(() =>
        askHeidi(client, token, sessionId, aiCommandText, content, contentType)
      );
    },
    [client, token, handleApiCall]
  );
